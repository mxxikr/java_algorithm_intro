...
| 핵심 개념           | 설명                                                            |
|--------------------|---------------------------------------------------------------|
| 그리디 정렬 기준     | 최대 회의 수를 위해 종료 시간 기준 오름차순으로 정렬, 종료 시간 같으면 시작 시간 빠른 순          |
| Find 연산 (Union-Find) | 요소가 속한 집합의 대표를 찾는 연산, 두 요소의 대표가 같으면 같은 집합임                    |
| Kruskal vs Prim    | Kruskal은 간선을 정렬해서 선택, Prim은 우선순위 큐 통해 트리 확장, 모두 사이클 발생하지 않도록 함 |
| 다익스트라의 가중치 조건 | 간선 가중치는 0 이상이어야 함, 음수 가중치 존재 시 올바른 결과 보장하지 못함                 |
| 동시 이벤트 처리 순서   | 도착과 출발이 동시에 있으면 출발 이벤트 먼저 처리해야 인원 계산이 올바름                     |


| 패턴      | 설명                                                                        |
|------------------|---------------------------------------------------------------------------|
| 그리디(Greedy)   | 매 단계에서 가장 좋아 보이는 선택을 하는 방법, 전체 최적을 보장하려면 각 단계의 선택이 이후에도 최적인지 확인 필요함       |
| Union-Find       | 여러 원소가 있을 때 어떤 두 원소가 같은 집합에 속해 있는지 혹은 두 집합을 합치는 연산을 효율적으로 처리하는 자료 구조      |
| 최소 스패닝 트리 | 그래프에서 모든 정점을 최소 비용으로 연결하는 트리(사이클 없음)를 찾는 알고리즘                             |
| 다익스트라(Dijkstra) | 하나의 정점에서 모든 정점으로의 최단 경로를 찾는 알고리즘, 간선 가중치가 음수이면 제대로 동작하지 않음                |
| 이벤트 처리 순서 | 여러 이벤트(도착/출발 등)가 동시에 발생할 때 정확한 결과 계산을 위해 적절한 순서로 처리해야 함 |

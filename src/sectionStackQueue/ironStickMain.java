...
public class ironStickMain {
    public int solution(String str) {
        ...
        for (int i = 0; i < str.length(); i++) {
            if (str.charAt(i) == '(') {
                stack.push('('); // 여는 괄호 일 경우 스택에 추가
            } else {
                stack.pop(); // 닫는 괄호 일 경우 스택에서 제거
                if (str.charAt(i - 1) == '(') { // 이전 문자가 여는 괄호인 경우 레이저임
                    answer += stack.size(); // 현재 스택의 크기를 더함(막대기 개수)
                } else { // 레이저가 아닌 경우는 쇠막대기의 끝
                    answer++; // 하나의 조각이 추가됨
                }
            }
        }
    ...
    public static void main(String[] args) {
        ironStickMain T = new ironStickMain(); // 객체 생성

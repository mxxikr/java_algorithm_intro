...
public class binaryTreeRoundMain {
    ...
    // 전위 순회 : 부모 -> 왼쪽 -> 오른쪽
    public void preOrder(NodeShortDFS root) {
        if (root == null) {
            return;
        } else {
    ...
    // 중위 순회 : 왼쪽 -> 부모 -> 오른쪽
    public void inOrder(NodeShortDFS root) {
        if (root == null) {
    ...
    // 후위 순회 : 왼쪽 -> 오른쪽 -> 부모
    public void postOrder(NodeShortDFS root) {
        if (root == null) {
            return; // 노드가 null이면 말단 노드이기 때문에 종료
        } else {
    ...
    public static void main(String[] args) {
        ...
        // 트리 구조 초기화
        tree.root = new NodeShortDFS(1); // 루트 노드 생성
        tree.root.lt = new NodeShortDFS(2); // 왼쪽 자식 노드 생성
        tree.root.rt = new NodeShortDFS(3); // 오른쪽 자식 노드 생성
        tree.root.lt.lt = new NodeShortDFS(4); // 왼쪽 자식의 왼쪽 자식 노드 생성
        tree.root.lt.rt = new NodeShortDFS(5); // 왼쪽 자식의 오른쪽 자식 노드 생성
        tree.root.rt.lt = new NodeShortDFS(6); // 오른쪽 자식의 왼쪽 자식 노드 생성
        tree.root.rt.rt = new NodeShortDFS(7); // 오른쪽 자식의 오른쪽 자식 노드 생성
